...
| **PII in responses** | You handle it | `[McpRedact]` + sanitization pipeline (GDPR) |
...
|----------|----------|-------------|
| **Omit** (default) | Remove property entirely | Email, phone, address |
| **Hash** | Stable SHA-256 | Correlation IDs |
| **Mask** | `j***@***.com` | UI-facing only (wastes LLM tokens) |
...
- **PII is omitted by default** — `[McpRedact]` with `Omit` strategy protects
  against accidental data leakage to AI agents
