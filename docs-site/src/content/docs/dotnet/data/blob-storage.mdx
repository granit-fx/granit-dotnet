---
title: Blob Storage
description: Multi-provider blob storage (S3, Azure, Google Cloud Storage, FileSystem, Database) with presigned URLs, proxy endpoints, validation pipeline, and tenant isolation.
sidebar:
  order: 16
---
...
| Extensions | `AddGranitBlobStorageS3()`, `AddGranitBlobStorageAzureBlob()`, `AddGranitBlobStorageGoogleCloud()`, `AddGranitBlobStorageFileSystem()`, `AddGranitBlobStorageDatabase()`, `AddGranitBlobStorageProxy()`, `AddGranitBlobStorageEntityFrameworkCore()`, `MapGranitBlobProxyEndpoints()` | -- |
