---
title: "Migrations"
description: Three migration approaches for Granit applications — the --migrate CLI mode for production deployments, standard EF Core for development, and Expand & Contract for zero-downtime breaking changes.
sidebar:
  label: Migrations
  order: 5
---
...
| | `--migrate` CLI mode | Standard (at startup) | SQL script (CI/CD) | Expand & Contract |
|---|---|---|---|---|
...
(respecting `[DependsOn]` declarations). Modules that do not implement
`IMigratableModule<T>` — including all internal Granit DbContexts such as
`BackgroundJobsDbContext` or `WebhooksDbContext` — are silently skipped. Only
**host application DbContexts** that own migration files should implement the interface.
...
For multi-instance scenarios where multiple pods could start simultaneously, register a
real distributed lock by implementing `IGranitMigrationLock`:
...
The recommended implementation for PostgreSQL uses advisory locks, which are
session-scoped and zero-dependency:

```csharp
// In your host application (e.g., Guava.Infrastructure)
public sealed class PostgresAdvisoryMigrationLock(NpgsqlDataSource dataSource)
    : IGranitMigrationLock
{
        ...
        string resource, CancellationToken cancellationToken)
    {
        ...
        return new LockHandle(connection, resource);
    }
}
```

Register it before `AddGranitMigrateSupport()` so `TryAddSingleton` defers to it:

```csharp
builder.Services.AddSingleton<IGranitMigrationLock, PostgresAdvisoryMigrationLock>();
builder.AddGranitMigrateSupport();
```

:::note
Advisory lock connections must use a dedicated `NpgsqlConnection`, not the EF Core
connection pool. Advisory locks are session-scoped — returning the connection to the
pool would silently release the lock.
:::
...
| **SaaS schema-per-tenant** | `ITenantEnumerator` iterates tenants; `ITenantDbIsolator` sets `search_path` |
