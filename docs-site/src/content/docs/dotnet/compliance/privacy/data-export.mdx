...
[DependsOn(
    ...
    typeof(GranitIdentityLocalWolverineModule),
    typeof(GranitAuditingWolverineModule),
    typeof(GranitNotificationsPrivacyWolverineModule))]
...
| ------ | ------------- | ------- | -------- | --- |
| `Identity.Local` | `identity-local` | `Granit.Identity.Local.Wolverine` | Profile + roles | — |
| `Identity.Federated` | `identity-federated` | `Granit.Identity.Federated.Wolverine` | Local cache entry (profile mirror + sync metadata) | — |
| `Auditing` | `auditing` | `Granit.Auditing.Wolverine` | User-authored audit trail | 10 000 entries |
| `Notifications` | `notifications` | `Granit.Notifications.Privacy.Wolverine` | Inbox + preferences + subscriptions | 5 000 inbox items |
...
├── manifest.json
├── identity-local.json        (from Granit.Identity.Local.Wolverine)
├── auditing.json              (from Granit.Auditing.Wolverine)
├── notifications.json         (from Granit.Notifications.Privacy.Wolverine)
