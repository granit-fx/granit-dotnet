...
By default, a null-object implementation is registered that throws
`NotImplementedException` — the application must provide a concrete implementation.
...
| `.IncludeExtraProperties()` | Append mapped extra properties (from `MapProperty<T>()`) as additional export fields at runtime |
...
mapped extra properties (`IHasExtraProperties` with `MapProperty<T>()`) — in that
...
- **Extra properties** — `DynamicReferenceDataEntity` uses `.IncludeExtraProperties()` to export
