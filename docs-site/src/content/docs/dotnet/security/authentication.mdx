...
`Granit.Authentication.OpenIddict` provides a `RequireDPoP` option that rejects any
request using the `Bearer` scheme when DPoP enforcement is active. Requests without
a valid `DPoP` proof header receive a `401 Unauthorized` response.
...
|----------|------|---------|-------------|
| `Issuer` | `Uri?` | `null` | OIDC issuer URI of the remote authorization server. |
| `Audience` | `string?` | `null` | Expected audience (resource server identifier). |
| `RequireDPoP` | `bool` | `false` | Require DPoP proof-of-possession (RFC 9449) for all authenticated requests. When `true`, plain `Bearer` tokens are rejected. Required for FAPI 2.0 compliance. |
