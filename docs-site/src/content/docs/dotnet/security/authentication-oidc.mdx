...
`Granit.Authentication.Oidc` is the foundation library for all OIDC and OAuth 2.0
...
packages like `Granit.Bff` and `Granit.Authentication.TokenManagement`, which
...
|--------------|---------|-----|
| Build or parse OIDC protocol messages (token requests, authorization URLs, discovery) | **Granit.Authentication.Oidc** | Foundation primitives -- no HTTP calls, no middleware |
...
| Manage token lifecycle (acquire, cache, refresh) for outgoing HTTP calls | `Granit.Authentication.TokenManagement` | Built on top of this package |
...
flowchart TD
    ...
    I -->|Yes| J["Granit.Authentication.TokenManagement<br/>(uses Oidc internally)"]
    I -->|No| K["Granit.Authentication.Oidc<br/>(protocol primitives only)"]
...
- Granit.Authentication.Oidc/
  - GranitAuthenticationOidcModule.cs Module registration
...
```csharp
[DependsOn(typeof(GranitAuthenticationOidcModule))]
...
:::note
You rarely need to depend on `GranitAuthenticationOidcModule` directly. Packages
like `Granit.Bff` and `Granit.Authentication.TokenManagement` already declare it
...
|----------|-----------|-------------|
| Module | `GranitAuthenticationOidcModule` | Registers `IDPoPProofService` + `IDiscoveryDocumentService` |
