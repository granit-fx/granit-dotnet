...
| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `DbTablePrefix` | `string` | `"oidc_"` | Table name prefix for all Identity and OpenIddict tables. |
...
All 23 permission constants are defined in `OpenIddictPermissions` and follow the
...
Configured by `AddGranitOpenIddictEntityFrameworkCore()`:
...
These are build-time defaults. The `MaxLoginAttempts` runtime setting
