...
Resource servers using `AddGranitOpenIddictAuthentication()` validate DPoP
...
alongside [PAR](/dotnet/security/openiddict/advanced/par/) and PKCE. The three
...
- [PAR (Pushed Authorization Requests)](/dotnet/security/openiddict/advanced/par/) — move parameters to back-channel
- [OIDC Server Configuration](/dotnet/security/openiddict/oidc-server/) — endpoints and flows overview
- [BFF Security](/dotnet/security/bff/security/) — token storage and encryption at rest
