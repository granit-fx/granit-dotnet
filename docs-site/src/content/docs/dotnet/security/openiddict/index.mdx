...
Comparable to [ABP Framework Account Pro](https://abp.io/docs/latest/modules/openiddict),
but designed for the Granit module system with multi-tenant isolation, Granit.Settings
per-tenant configuration, and Wolverine-based background jobs.
...
- Granit.OpenIddict/ Abstractions, entities, options, permissions, events
  ...
  - Granit.Identity.OpenIddict/ IIdentityProvider bridge (AspNetIdentityProvider)
...
| `Granit.Identity.OpenIddict` | `AspNetIdentityProvider` implementing all 7 `IIdentityProvider` sub-interfaces |
...
graph TD
    ...
    OID --> SEC[Granit.Security]
    OID --> QRY[Granit.Querying]
    ...
    IOID[Granit.Identity.OpenIddict] --> IDEN
    IOID --> EFC
...
```csharp
[DependsOn(
    typeof(GranitIdentityOpenIddictModule),
...
All Identity and OpenIddict tables use the `oidc_` prefix (configurable via
...
- [OIDC Server Configuration](./oidc-server/) -- flows, endpoints, signing keys
- [User Management](./user-management/) -- entities, admin API, IIdentityProvider bridge
- [Account Self-service API](./account-api/) -- registration, 2FA, GDPR
