...
|--------|-----------|-------|
| `GranitOpenIddictApplication` | `OpenIddictEntityFrameworkCoreApplication<Guid, ...>` | `oidc_applications` |
| `GranitOpenIddictAuthorization` | `OpenIddictEntityFrameworkCoreAuthorization<Guid, ...>` | `oidc_authorizations` |
| `GranitOpenIddictScope` | `OpenIddictEntityFrameworkCoreScope<Guid>` | `oidc_scopes` |
| `GranitOpenIddictToken` | `OpenIddictEntityFrameworkCoreToken<Guid, ...>` | `oidc_tokens` |

Applications and scopes with `TenantId == null` are global (visible to all tenants).

## IIdentityProvider bridge

`Granit.Identity.OpenIddict` registers `AspNetIdentityProvider` as the
...
`AddGranitOpenIddictEntityFrameworkCore()` configures ASP.NET Core Identity with
secure defaults:
...
| `POST` | `/` | `Scopes.Create` | Register a new scope |
...
| Method | Path | Permission | Description |
|--------|------|------------|-------------|
| `GET` | `/` | `Authorizations.Read` | List authorizations (`?userId`, `?clientId`) |
| `DELETE` | `/{authorizationId}` | `Authorizations.Revoke` | Revoke authorization + tokens |
| `DELETE` | `/user/{userId}` | `Authorizations.Revoke` | Revoke all for user (GDPR / security) |

## Database tables

All tables are prefixed with `oidc_` (configurable via `GranitOpenIddictDbProperties.DbTablePrefix`).
The schema is configurable via `GranitOpenIddictDbProperties.DbSchema`.

| Table | Entity | Multi-tenant |
|-------|--------|-------------|
| `oidc_users` | `GranitUser` | Yes |
| `oidc_roles` | `GranitRole` | No (global) |
| `oidc_user_roles` | `IdentityUserRole<Guid>` | -- |
| `oidc_user_claims` | `IdentityUserClaim<Guid>` | -- |
| `oidc_user_logins` | `IdentityUserLogin<Guid>` | -- |
| `oidc_user_tokens` | `IdentityUserToken<Guid>` | -- |
| `oidc_role_claims` | `IdentityRoleClaim<Guid>` | -- |
| `oidc_user_groups` | `GranitUserGroup` | Yes |
| `oidc_user_group_members` | `GranitUserGroupMember` | Yes |
| `oidc_applications` | `GranitOpenIddictApplication` | Yes |
| `oidc_authorizations` | `GranitOpenIddictAuthorization` | Yes |
| `oidc_scopes` | `GranitOpenIddictScope` | Yes |
| `oidc_tokens` | `GranitOpenIddictToken` | Yes |

### Notable indexes

| Index | Type | Columns |
|-------|------|---------|
| `ix_oidc_users_tenant_id` | Non-unique | `TenantId` |
| `uq_oidc_user_groups_tenant_name` | Unique | `TenantId`, `Name` |
| `uq_oidc_user_group_members_group_user` | Unique | `GroupId`, `UserId` |
| `ix_oidc_user_group_members_user_id` | Non-unique | `UserId` |
