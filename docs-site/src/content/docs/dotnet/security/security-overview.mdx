...
| You need external secret/key management | [Vault & Encryption](/dotnet/data/vault-encryption/) | Production environments — never store encryption keys in config files |
...
graph TD
    ...
    GC[Granit.Authentication.GoogleCloud] --> JWT
