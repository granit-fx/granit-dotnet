...
`Granit.Authentication.TokenManagement` solves this by providing a `DelegatingHandler`
...
| **Machine-to-machine API calls** | **`Granit.Authentication.TokenManagement`** | **Client credentials** | **Backend services, workers, jobs** |
| Low-level token endpoint operations | `Granit.Authentication.Oidc` | Any grant type | Custom flows, migration code |

:::tip
If your service calls another API on behalf of itself (not a user), Token Management
is the right choice. If you need to forward a user's token or exchange it via
`on_behalf_of`, use the BFF pattern or implement a custom flow with `ITokenEndpointService`.
:::
...
- Granit.Authentication.TokenManagement/
  ...
  - GranitAuthenticationTokenManagementModule.cs Module entry point

</FileTree>

| Dependency | Role |
|------------|------|
| `Granit.Authentication.Oidc` | Discovery, token request encoding, DPoP proof generation, client authentication strategies |
| `Granit.Core` | Module system, `GranitActivitySourceRegistry` |
...
```csharp
[DependsOn(typeof(GranitAuthenticationTokenManagementModule))]
...
communication. `GranitAuthenticationTokenManagementModule` depends on
`GranitAuthenticationOidcModule` and `GranitTimingModule` — both are loaded transitively.
...
The `PrivateKeyJwtStrategy` (from `Granit.Authentication.Oidc`) generates a short-lived
...
All counters are registered under the `Granit.Authentication.TokenManagement` meter
and follow the Granit metrics conventions.

| Metric | Type | Tags | Description |
|--------|------|------|-------------|
| `granit.authentication.token.requests` | Counter | `tenant_id`, `grant_type`, `client_name` | Token endpoint requests sent |
| `granit.authentication.token.cache_hits` | Counter | `tenant_id`, `client_name` | Token served from cache |
| `granit.authentication.token.cache_misses` | Counter | `tenant_id`, `client_name` | Token not in cache, endpoint call required |
| `granit.authentication.token.revocations` | Counter | `tenant_id` | Successful token revocations |
| `granit.authentication.token.errors` | Counter | `tenant_id`, `error_type` | Token endpoint errors (network, protocol) |

### Distributed tracing

The `Granit.Authentication.TokenManagement` `ActivitySource` emits spans for:
