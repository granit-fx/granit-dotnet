...
### Opt-in via the permission's `MultiTenancySide`

Host access is declared **once**, on the permission itself, via
[`MultiTenancySide`](/dotnet/security/authorization-multitenancy-side/):

```csharp title="SubscriptionsPermissionDefinitionProvider.cs"
group.AddPermission(
    SubscriptionsPermissions.Subscriptions.Read,
    L("Permission:Subscriptions.Read"),
    MultiTenancySide.Both); // host admin + tenant can both call
```

Endpoints gated by this permission are automatically accessible in either context:

```csharp
group.MapGet("/{id:guid}", GetByIdAsync)
    .WithName("GetSubscription")
    .RequireAuthorization(SubscriptionsPermissions.Subscriptions.Read);
    // No extra marker — the permission's side already opts the endpoint in.
```

`IPermissionChecker` enforces the side at check time:

- Permission includes `Host` and no tenant is active → allowed (authenticated caller
  required, as always)
- Permission includes `Tenant` and a tenant is active → allowed
- Mismatch → denied at check time, before any data access

### Two-layer defense

| Layer | Component | Role |
|-------|-----------|------|
| **Authorization** | `.RequireAuthorization(perm)` → `IPermissionChecker` | Enforces `MultiTenancySide` + grant lookup (per-provider `perm:{tenantId\|"global"}:...` cache key) |
| **Data** | `EfStoreBase.Query(db)` | `IgnoreQueryFilters([MultiTenant])` when `!currentTenant.IsAvailable` |

A tenant user who omits the `X-Tenant-Id` header hits the safe default (no data
returned) on every endpoint. A host caller lacking the right permission gets
**403 Forbidden** from the authorization layer. A tenant caller invoking a
`Host`-sided permission is also denied at check time.

### When to use `MultiTenancySide.Both`

Tag `Both` on **read** permissions where host administrators need cross-tenant
visibility (list/get by id/admin dashboards) **and** tenants need their own view
of the same data — the typical SaaS "host bills tenant" surfaces (Invoicing,
Payments, CustomerBalance, Subscriptions, Tax).

Use `Host` for operations that are only meaningful at the platform level: tenant
provisioning, impersonation, audit pruning, global feature-flag definitions,
OIDC scopes, diagnostics, recurring job scheduler controls.

Use `Tenant` for operations tied to a specific tenant's data model (per-tenant
settings, and anything a host admin should **not** be able to do without entering
