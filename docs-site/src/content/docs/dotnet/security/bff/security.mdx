...
Granit.Bff uses **absolute expiry** by default. The session duration starts
from the moment of login and is not extended by activity. This is a deliberate
security choice:

- **Absolute**: forces periodic re-authentication (ISO 27001 A.9.4.2 recommends
  maximum session durations)
- **Sliding**: extends with activity, which could keep a stolen session alive
  indefinitely

If your use case requires sliding expiry, you can implement a custom
`IBffTokenStore` that refreshes the cache entry TTL on each `GetAsync` call.

### Session revocation

To immediately invalidate a session (e.g., when an admin revokes a user's access):

```csharp
// Inject IDistributedCache and remove the session directly
await cache.RemoveAsync($"bff:session:{sessionId}", cancellationToken);
```

This is immediate — the next API call from the SPA will receive `401 Unauthorized`
because the token lookup in `BffTokenInjectionTransform` returns `null`.

The user does not need to be notified — the SPA's existing `401` handler will
redirect to `/bff/login`.

<Aside type="tip" title="Bulk session revocation">
To revoke all sessions for a user (e.g., password change), you need to track
session IDs per user. Store a reverse mapping (`bff:user:{userId}` -> list of
session IDs) in Redis and iterate to remove them.
</Aside>
...
| **Token replay** | Short-lived access tokens | IdP-configured expiry + auto-refresh |
...
- **[YARP Proxy](/dotnet/security/bff/yarp-proxy/)** — route configuration and token injection pipeline
