...
graph TD
    ...
    BFF --> CORE[Granit.Core]
    BFF --> SEC[Granit.Security]
...
- **[Security](/dotnet/security/bff/security/)** — cookie hardening, CSRF protection, clickjacking, session management
- **[YARP Proxy](/dotnet/security/bff/yarp-proxy/)** — route configuration, token injection pipeline, multi-service routing
