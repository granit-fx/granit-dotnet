...
| `Granit.AI.Endpoints` | Workspace CRUD, usage query, chat/embedding proxy | `Granit.AI`, `Granit.Authorization`, `Granit.QueryEngine.Endpoints` |
...
[Granit.Vault](/dotnet/data/vault-encryption/) with dynamic rotation.
:::
...
| **Querying.AI** | Natural language → structured filters ("unpaid invoices from last week") | [AI: Natural Language Query](/dotnet/ai/natural-language-query/) |
...
### `QueryEngineAIOptions` (section: `AI:Querying`)
...
- [Querying](/dotnet/business/querying/) — query engine where NLQ translates natural language
- [Persistence](/dotnet/data/persistence/) — isolated DbContext pattern, `ApplyGranitConventions`
- [Vault & Encryption](/dotnet/data/vault-encryption/) — secure API key storage with rotation
