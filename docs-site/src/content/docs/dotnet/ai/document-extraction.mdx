...
`Granit.AI.Extraction` takes a different approach: send the document text to an LLM with a
JSON schema describing the expected output, and get back a strongly-typed C# object. The LLM
handles layout variations, multilingual content, and messy formatting — you get clean data.
...
|------|--------|-----------|
| **Hallucination** | LLM invents data not in the document | Structured JSON output constrains the response; FluentValidation catches inconsistencies |
...
- [Granit.AI overview](/reference/modules/ai/) — core module, providers, workspaces
- [AI: Import Mapping](/ai/import-mapping/) — AI for CSV/Excel column mapping
- [AI: Semantic Search](/ai/semantic-search/) — index extracted content for RAG
