...
`AuditedAggregateRoot, IWorkflowStateful, IHasExtraProperties` — the catalog item.
...
- `ExtraPropertiesJson` (string?) — Stripe-style metadata via `IHasExtraProperties`;
  use the framework extensions (`GetExtraProperty`, `SetExtraProperty`, ...)
...
- `ReplaceExtraProperties(dict)` — bulk replace; for granular updates, use the
  framework `SetExtraProperty(name, value)` extension
...
- **Published**: immutable except for `ExtraProperties` and `ExternalMappings`;
...
- `PUT  /catalog/products/{id}/extra-properties` — bulk-replace extra properties
