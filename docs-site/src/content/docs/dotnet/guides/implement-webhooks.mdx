...
internal sealed class AppWebhookEventTypeProvider : IWebhookEventTypeDefinitionProvider
{
    public void Define(IWebhookEventTypeDefinitionContext context)
    {
        ...
        context.Add("order.shipped", "Order shipped",
            "Fired when an order ships.", "Orders");
        context.Add("invoice.created", "Invoice created",
            "Fired when an invoice is generated.", "Billing");
    }
}
```
...
- **Discovery endpoint** — `GET /event-types` returns the list for admin UIs
