...
1. **HeaderTenantResolver** (Order 100) -- reads the `X-Tenant-Id` header, used for
   service-to-service calls
2. **JwtClaimTenantResolver** (Order 200) -- reads the `tenant_id` claim from the JWT token,
...
:::note[Good to know]
`ICurrentTenant` is defined in `Granit.Core.MultiTenancy` and available in every
...
- [Configure caching](/guides/configure-caching/) -- set up distributed caching with tenant-aware keys
- [Create a module](/guides/create-a-module/) -- build a tenant-aware module from scratch
- [Multi-tenancy reference](/reference/modules/multi-tenancy/) -- full API and architecture details
