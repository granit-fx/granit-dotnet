...
The migration creates the `core_setting_records` table with a unique index on
...
- [Encrypt sensitive data](/guides/encrypt-sensitive-data/) -- encrypt settings with `IsEncrypted = true`
- [Add feature flags](/guides/add-feature-flags/) -- SaaS feature management with quota guards
