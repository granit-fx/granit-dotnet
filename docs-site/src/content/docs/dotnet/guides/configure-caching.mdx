...
2. **Register both modules**
   ...
   `GranitCachingRedisModule` depends on `GranitCachingModule` -- if you only
   declare `GranitCachingRedisModule`, the base module is pulled in automatically.
...
Check that `Cache:Redis:IsEnabled` is `true` (default). `GranitCachingRedisModule`
