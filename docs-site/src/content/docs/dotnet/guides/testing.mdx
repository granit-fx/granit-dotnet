...
|---------|------|------------|
| `Granit.Testing` | `GranitTestFixture`, fakes, `GranitWebApplicationFactory`, Bogus generators | `Granit.Core`, `Granit.Security`, `Granit.Timing`, `Granit.Guids`, `Bogus` |
...
```mermaid
graph TD
    T[Granit.Testing] --> C[Granit.Core]
    T --> S[Granit.Security]
    T --> Ti[Granit.Timing]
    T --> G[Granit.Guids]
...
public sealed class CachingTests : IAsyncDisposable
{
    ...
    public async Task Cache_Service_Is_Resolved()
    {
        ...
        var cache = _fixture.GetRequiredService<ICacheService<string>>();
