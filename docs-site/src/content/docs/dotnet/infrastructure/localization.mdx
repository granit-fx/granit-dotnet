...
Translation overrides are cached in-memory **per tenant**. If you update an override
via the CRUD endpoints, the change is visible immediately for that tenant. Other tenants
or other pods see the change after the cache expires (controlled by the caching module TTL).
:::
