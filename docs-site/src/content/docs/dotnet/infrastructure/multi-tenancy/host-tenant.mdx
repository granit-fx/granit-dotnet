...
Granit provides **2 layers** that work off a single declarative source — the
permission's [`MultiTenancySide`](/dotnet/security/authorization-multitenancy-side/):

1. **Authorization layer** — `.RequireAuthorization(permission)` goes through
   `IPermissionChecker`, which rejects the call when the required permission's
   `MultiTenancySide` doesn't match the current tenant context. A permission tagged
   `Host` or `Both` is the single declarative opt-in that allows host-context callers
   to reach the endpoint. Authentication is already enforced by the checker
   (VULN-001), so no separate host-access filter is needed.
2. **Data layer** — `EfStoreBase.Query(db)` bypasses the `MultiTenant` named query
   filter per-query via `IgnoreQueryFilters` when `ICurrentTenant.IsAvailable` is
   `false`, without affecting other filters.

```csharp
// Endpoint: host access derived from the permission's MultiTenancySide.
// No extra marker needed on the endpoint itself.
group.MapGet("/{id:guid}", GetByIdAsync)
    .RequireAuthorization(InvoicingPermissions.Invoices.Read);
    // InvoicingPermissions.Invoices.Read is tagged MultiTenancySide.Both,
    // so the host is allowed to call this endpoint.
