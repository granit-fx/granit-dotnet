...
dispatched as a domain event. The `Granit.MultiTenancy.Wolverine` package provides a
...
```bash
dotnet add package Granit.MultiTenancy.Wolverine
```

```csharp
[DependsOn(typeof(GranitMultiTenancyWolverineModule))]
