...
The pre-1.0 hardening pass removed `BrowserPageOptions.BypassCsp`. Disabling
CSP now requires both a sandbox with `ForceCsp = false` *and* the
`Granit.Browsing.Pages.BypassCsp` permission on the caller's role — see the
[migration guide](/dotnet/browsing/migration-pre-1-0-hardening/).
