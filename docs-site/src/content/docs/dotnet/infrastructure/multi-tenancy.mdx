...
|---------|------|------------|
| `Granit.MultiTenancy` | Tenant resolution, `CurrentTenant`, middleware | `Granit` |

:::note
`ICurrentTenant` lives in `Granit.MultiTenancy` and is available in **every** module
...
`Granit.Http.ApiDocumentation` automatically excludes the `X-Tenant-Id` header from
...
:::note[Interceptor wiring]
All three per-tenant factories (`SharedDatabaseDbContextFactory`, `TenantPerSchemaDbContextFactory`,
`TenantPerDatabaseDbContextFactory`) call `UseGranitInterceptors()` internally, wiring the
full interceptor chain — audit, versioning, concurrency stamp, domain events, entity lifecycle
events, and soft delete — identically to `AddGranitDbContext<T>()`.
:::
...
In `CrossValidate` mode, when the user is authenticated and has a `tenant_id`
JWT claim, the resolved tenant must match the claim. A mismatch returns
**403 Forbidden** and logs a warning with the resolved and expected tenant IDs.

| Mode | Behavior | Use case |
|------|----------|----------|
| `Unrestricted` | Header accepted as-is | Behind BFF/reverse proxy |
| `CrossValidate` | Header must match JWT claim | Direct API exposure |

### Observability

The middleware emits three OpenTelemetry counters under the `Granit.MultiTenancy` meter:

| Metric | Tags | Description |
|--------|------|-------------|
| `granit.multi_tenancy.resolution.succeeded` | `tenant_id`, `resolver_type` | Successful tenant resolutions |
| `granit.multi_tenancy.resolution.failed` | `tenant_id` | Failed resolutions (no resolver matched or cross-validation rejected) |
| `granit.multi_tenancy.context.switched` | `tenant_id` | Explicit `ICurrentTenant.Change()` calls |

The `resolver_type` tag identifies which resolver matched (`HeaderTenantResolver`,
`JwtClaimTenantResolver`, or a custom resolver), enabling targeted dashboards.
...
| `HeaderTrustMode` | `"Unrestricted"` | `Unrestricted` or `CrossValidate` (match header against JWT claim) |
...
| Core (in Granit) | `ICurrentTenant`, `AllowAnonymousTenantAttribute` | `Granit` |
| Implementation | `CurrentTenant`, `TenantInfo`, `ITenantInfo` | `Granit.MultiTenancy` |
| Resolution | `ITenantResolver`, `HeaderTenantResolver`, `JwtClaimTenantResolver`, `TenantResolverPipeline`, `TenantResolutionResult` | `Granit.MultiTenancy` |
| Middleware | `TenantResolutionMiddleware` | `Granit.MultiTenancy` |
| Options | `MultiTenancyOptions`, `TenantHeaderTrustMode` | `Granit.MultiTenancy` |
| Diagnostics | `MultiTenancyMetrics` | `Granit.MultiTenancy` |
