...
<FileTree>
- Granit.Core/Events/ Interfaces — always available, zero dependencies
...
- Granit.EventBus/ Default in-process providers (monolith, tests)
...
- Granit.EventBus.Wolverine/ Wolverine-backed providers (production)
...
| ------- | ---- | ----------- |
| `Granit.Core` | `ILocalEventBus`, `IDistributedEventBus` interfaces | Always available — no extra dependency |
| `Granit.EventBus` | `InProcessLocalEventBus`, `InProcessDistributedEventBus` | Monoliths, unit tests, development |
| `Granit.EventBus.Wolverine` | `WolverineLocalEventBus`, `WolverineDistributedEventBus` | Production — outbox-backed delivery |
...
| Feature flag change logging | Local | In-process handler via `Granit.AuditLog.ConfigurationChanges` |
...
`INotificationPublisher`, `IMigrationBatchDispatcher`.
:::
...
public sealed class OrderPlacedAuditHandler(IAuditLogWriter writer) : ILocalEventHandler<OrderPlacedEvent>
{
...
{
    public async Task HandleAsync(OrderShippedEvent integrationEvent, CancellationToken ct = default)
    {
...
await distributedEventBus.PublishAsync(
    new OrderShippedEvent(orderId, tracking, timeProvider.GetUtcNow()), ct);
```
...
<Tabs>
  <TabItem label="In-process (manual DI)">
    With `Granit.EventBus` (in-process provider), handlers are resolved from DI.
    You must register them explicitly:

    ```csharp
    // In your module's ConfigureServices or DI extension:
    services.AddScoped<ILocalEventHandler<OrderPlacedEvent>, OrderPlacedAuditHandler>();
    services.AddScoped<IDistributedEventHandler<OrderShippedEvent>, ShippingNotificationHandler>();
    ```

    Multiple handlers per event type are supported — all are called sequentially.
  </TabItem>
  <TabItem label="Wolverine (auto-discovery)">
    With `Granit.EventBus.Wolverine`, handlers that follow Wolverine's naming
...
2. Is `AddGranitEventBus()` called? (auto-called by Settings, Features, DataExchange)
3. For distributed: is `Granit.EventBus.Wolverine` loaded? (check module dependencies)
...
<Tabs>
  <TabItem label="In-process (default)">
    ...
    *"InProcessDistributedEventBus is not durable — events will be lost on crash."*
  ...
  <TabItem label="Wolverine (production)">
    ...
    // Granit.EventBus.Wolverine auto-replaces via [DependsOn] module system
    ```
...
**Naming**: use `XxxEvent` (e.g., `OrderShippedEvent`). Not `XxxDto`, not `XxxMessage`.
...
public async Task OrderPlacedAuditHandler_PersistsAuditEntry()
{
    ...
    services.AddGranitEventBus();
    services.AddScoped<ILocalEventHandler<OrderPlacedEvent>, OrderPlacedAuditHandler>();
    services.AddScoped<IAuditLogWriter, FakeAuditLogWriter>();
    ...
    // Assert
    FakeAuditLogWriter writer = (FakeAuditLogWriter)sp.GetRequiredService<IAuditLogWriter>();
...
Always install `Granit.EventBus.Wolverine` with an outbox provider for production
distributed events.
:::
...
| Distributed event lost on crash | Using in-process provider | Install `Granit.EventBus.Wolverine` + outbox |
...
| -------- | --------- | ------- |
| Local bus | `ILocalEventBus`, `ILocalEventHandler<T>` | `Granit.Core` |
| Distributed bus | `IDistributedEventBus`, `IDistributedEventHandler<T>` | `Granit.Core` |
| Integration marker | `IIntegrationEvent` | `Granit.Core` |
| In-process providers | `InProcessLocalEventBus`, `InProcessDistributedEventBus` | `Granit.EventBus` |
| Wolverine providers | `WolverineLocalEventBus`, `WolverineDistributedEventBus` | `Granit.EventBus.Wolverine` |
| DI extension | `AddGranitEventBus()` | `Granit.EventBus` |
| Modules | `GranitEventBusModule`, `GranitEventBusWolverineModule` | respective packages |
...
graph TD
    ...
    subgraph "Event Bus Abstraction (Granit.Core)"
        LEB[ILocalEventBus]
...
- [Audit Log](/dotnet/security/audit-log/) — audit trail leveraging local event bus
- [Settings](/dotnet/infrastructure/settings/) — publishes `SettingChangedEvent` via local bus
- [Features](/dotnet/infrastructure/features/) — publishes `FeatureOverrideChangedEvent` via local bus
- [Wolverine](/dotnet/infrastructure/wolverine/) — message bus, outbox, and context propagation
