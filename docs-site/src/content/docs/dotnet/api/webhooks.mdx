...
internal sealed class OrderWebhookEventTypeProvider : IWebhookEventTypeDefinitionProvider
{
    ...
    {
        context.Add("order.created", "Order created",
            "Fired when a new order is placed.", "Orders");
        context.Add("order.shipped", "Order shipped",
            "Fired when an order is marked as shipped.", "Orders");
        context.Add("order.cancelled", "Order cancelled",
            "Fired when an order is cancelled by the customer or an admin.", "Orders");
    }
}
```

The definitions are collected into an immutable `IWebhookEventTypeRegistry` singleton,
pre-sorted by category then name. The registry serves two purposes:
...
:::tip
Each `WebhookEventTypeDefinition` accepts an optional `DisplayName`, `Description`,
and `Category`. Providing all three enables richer UI experiences — category-based
grouping and human-readable labels in dropdown menus.
:::
