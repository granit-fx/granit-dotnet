...
`ApplyGranitConventions` scans all entity types in the model and registers
...
EF Core supports only one `HasQueryFilter` per entity — the last registration
wins. Manual filters silently override the framework's combined filter,
breaking tenant isolation or soft delete. Let `ApplyGranitConventions`
own all query filters.
:::
...
- [Persistence reference](/reference/modules/persistence/) — configuration
