...
### Changed (framework — BREAKING: auditing pipeline overhaul)

- **Single durable audit persistence pipeline — the in-memory Channel and the Strict/Async mode duo are gone.** `Granit.Auditing` no longer buffers audit batches in a `Channel<T>` drained by a hosted worker (a pipeline that lost buffered entries on pod kill and dropped batches after ~6s of DB outage). Persistence now has two self-selecting modes behind one choke point (`AuditPersistencePipeline`):
  - **Embedded (atomic)** — when the audited host `DbContext` maps the audit entities via `modelBuilder.ConfigureAuditingModule()`, the interceptor stages the `AuditEntry` graph into the **same transaction** as the business mutation: a rollback removes the audit rows with it, and the `AuditEntryPersistedEto` rides the pre-commit integration-event dispatch (Wolverine outbox envelope committed atomically with the row).
  - **Standalone (fallback)** — hosts that don't map the audit tables keep today's post-commit synchronous write through the isolated `AuditingDbContext` (not atomic; a one-shot warning per context type invites mapping). Nobody regresses; atomic is an opt-in upgrade.
  - ⚠️ **Exactly ONE DbContext may own the audit-table DDL.** Additional audited contexts must map with `ConfigureAuditingModule(excludeFromMigrations: true)` or migration generation/deployment crashes on duplicate tables.
  - **Removed types**: `AuditPersistenceMode`, `IAuditEntryPublisher`, `IAuditBatchPersister`, `ChannelAuditingPublisher`, `AuditingPersistenceWorker`, `StrictAuditingPublisher`, `EfCoreAuditBatchPersister`, `AuditingChannelHealthCheck` + `AddGranitAuditingChannelHealthCheck` (in embedded mode audit-write health *is* host-DB health, already probed).
  - **Removed configuration keys**: `Auditing:PersistenceMode`, `Auditing:ChannelCapacity` (now silently ignored by the binder — clean them from `appsettings.json`).
  - **`SaveChanges` return value** now includes the audit rows (+1 entry, +N children) on hosts using embedded mode — adjust any code asserting exact row counts.
  - **Bug fixes folded in**: change-tracking capture was **silently dead in every standard host wiring** — the interceptor resolved `ChangeTrackingCaptureService` via EF's *internal* service provider (`((IInfrastructure<IServiceProvider>)context).Instance.GetService<T>()`), which never resolves application services, so `SaveChanges` produced **zero audit entries** with no exception and no log; resolution now falls back through `CoreOptionsExtension.ApplicationServiceProvider` (the chain EF's own `context.GetService<T>()` walks). Also: `AuditEntryPersistedEto.Id` is now always the real persisted `AuditEntry.Id` (the async worker used to fabricate a fresh Guid — SIEM correlation and the anomaly-notification deep-link were broken in Async mode); explicit `IAuditingWriter` writes (auth events, access-denied) finally emit the Eto (they never did — `Granit.Auditing.Notifications` now receives the very events it exists for); a failed business save detaches the staged audit graph so a retry after `DbUpdateConcurrencyException` cannot double-write audit rows.
  - **Metrics**: `AuditingMetrics` ctor is now `(IMeterFactory)`; `granit.auditing.channel.depth` and `granit.auditing.persistence.retries` removed; `granit.auditing.batch.size` renamed `granit.auditing.entry.entity_changes`; `granit.auditing.persistence.duration` gains a `mode` tag (`embedded`/`standalone`); `granit.auditing.entry.persisted` now increments on every path (it silently reported zero in Strict mode, the default).
- **Audit retention is now data, with an ISO 27001 floor.** The five parallel `Auditing:*Retention` properties are replaced by a `Auditing:Retention` dictionary keyed by `AuditCategory` (env-overridable per category: `Auditing__Retention__DataAccess=…`); categories absent from the dictionary fall back to built-in defaults — a partial override can never create a retention gap, and the previously **unvalidated** `PrivilegedAccessRetention` (a `TimeSpan.Zero` misconfiguration would have purged the entire privileged-access trail on the first cleanup run) is now validated like every other category. Defaults rise to the ISO 27001 3-year minimum: `DataMutation` 365d → **1095d**, `DataAccess` 90d → **1095d** (ConfigurationChange/AccessDenied/PrivilegedAccess stay at 2555d). The validator floor is the new `Auditing:MinimumRetention` (default 1095d) — lowering it is an explicit, documented compliance deviation.
- **GDPR Art. 17 erasure now pseudonymizes the audit trail's direct identifiers by default.** `AuditingPersonalDataDeletionHandler` no longer acknowledges with "retained, 0 records": it calls `IAuditingCleaner.PseudonymizeByUserAsync` (user id hashed, username masked, IP/user-agent cleared — events retained per Art. 17(3)(b) + ISO 27001 A.12.4) and reports `Anonymized` with the real count. Opt out via `Auditing:PseudonymizeOnErasure = false`. The pseudonymization hash is now **salted** via `Auditing:PseudonymizationSalt` (provide via env/Vault; unsalted hashing of low-entropy user ids is re-identifiable and now logs a warning). The manual `POST /auditing/audit-entries/pseudonymize/{userId}` endpoint remains and the operation finally emits its `auditing.pseudonymize` trace span.
- **Cross-tenant leak fixed on `AuditEntityChange`.** The entity is now `IMultiTenant` with the tenant id denormalized from its parent `AuditEntry` (EF's multi-tenant query filter does not propagate through a parent join, so the query-engine/export surface over `audit_log_entity_changes` returned cross-tenant rows to tenant-scoped callers). **Hosts must add a migration**: new nullable `TenantId` column on `audit_log_entity_changes` + backfill from the parent, e.g. `UPDATE audit_log_entity_changes c SET "TenantId" = e."TenantId" FROM audit_log_log_entries e WHERE c."AuditLogEntryId" = e."Id";`. The same migration pass picks up the new `(Category, Timestamp)` purge index, the new `CorrelationId` index, and the `OriginalValue`/`NewValue` column-type change (explicit legacy `text` dropped — SQL Server hosts move to `nvarchar(max)`).
- **Auditing API contract changes** (`Granit.Auditing.Endpoints` / `IAuditingReader`): `GET /auditing/audit-entries/correlation/{correlationId}` is now **paginated** and returns the summary projection (`PagedResult<AuditEntryResponse>` with `page`/`pageSize`, was an unbounded `List<AuditEntryDetailResponse>`); `IAuditingReader.GetByCorrelationIdAsync` returns `PagedResult<AuditEntry>`. `AuditEntryDetailResponse.Category` and `AuditEntityChangeResponse.ChangeType` are typed enums again (wire JSON unchanged — clients regain a real enum schema in OpenAPI). Problem-detail messages are localized (`Granit:Auditing:Endpoints:*`, 15 base cultures + Workspace nav strings translated in all of them). `AuditingEndpointsOptions` now binds from `Auditing:Endpoints` (`ValidateOnStart`); the schema example provider is actually registered (it was dead code — audit examples never reached the OpenAPI document).
- **Module anatomy**: new `GranitAuditingAbstractionsModule` marker (aligns the 8 `.Abstractions` packages); `[DependsOn]` resynchronized with the reference graph on `GranitAuditingModule` and `GranitAuditingEndpointsModule`; unused `Granit.Timing` reference dropped from the base package; `AuditNotificationOptions` renamed `AuditingNotificationsOptions` (SectionName `Auditing:Notifications` unchanged).

### Changed (framework — BREAKING: unified migration orchestration)

- **One migration code path, one lock (#3167).** `MigrationStartupService` no longer runs its own resume pipeline with its own lock: it now delegates to `IGranitMigrationRunner` in the new `MigrationRunMode.ResumeBatches` mode, sharing the runner's distributed lock (single resource `"GranitMigration"` — the `"GranitMigrationStartup"` resource no longer exists), timeout, and exit-code semantics. The resume-command dispatch itself moved to the new public seam `IMigrationBatchResumer` (registered by `AddGranitPersistenceMigrations()`). Hosts using `AddGranitPersistenceMigrations()` **without** `AddGranitMigrateSupport()` no longer resume pending cycles at startup — a warning is logged pointing at the missing registration.
  - **`IGranitMigrationRunner` moved** from `Granit.Persistence.EntityFrameworkCore.Hosting` to `Granit.Persistence.EntityFrameworkCore.Migrations`, and `RunAsync` gained a leading `MigrationRunMode` parameter (defaulted — `RunAsync(ct)` call sites must become `RunAsync(cancellationToken: ct)` or `RunAsync(MigrationRunMode.Full, ct)`).
- **`GranitMigrateOptions` joins the IOptions pattern (`Persistence:Migrate`).** The class (still in `...Hosting.Options`) now exposes `SectionName = "Persistence:Migrate"` and is bound by `AddGranitMigrateSupport()` via `BindConfiguration` + `ValidateDataAnnotations` + `ValidateOnStart`; `Timeout`, `MaxRetries`, `RetryDelay`, `SeedAfterMigration`, `SeedOnStartup` and `CliFlag` are finally configurable from `appsettings.json`/env vars. The `configure` delegate still wins over configuration. The **bare `GranitMigrateOptions` singleton registration is gone** — resolve `IOptions<GranitMigrateOptions>` instead.
- **The migration connection string name is configurable.** New `Persistence:Migrate:ConnectionStringName` (default `"DefaultConnection"`) is honored by the runner's schema-ensuring passes and by both provider distributed locks (`pg_try_advisory_lock`, `sp_getapplock`) — no more hard-coded `"DefaultConnection"` in the migration path.
- **The migration lock fails closed (#3168).** New `Persistence:Migrate:RequireDistributedLock` (default: **required outside Development**; unknown environment counts as production). When it resolves to `true`: missing lock prerequisites (connection string, `DbProviderFactory`) make the provider locks **throw** instead of returning a silent no-op handle, and the runner refuses to run (exit 1, Critical log) when only the `NullMigrationLock` fallback is registered. The pre-#3168 behavior — a misconfigured host migrating unlocked and exiting 0 — requires an explicit `RequireDistributedLock: false` (single-instance hosts only).
- **Postgres advisory-lock key: `hashtext()` → stable bigint, with a rolling-upgrade transition (#3168).** The lock key is now the first 8 bytes (big-endian) of `SHA-256(resource)` — contractual across PostgreSQL versions, 64-bit (vs the internal, int32, collision-prone `hashtext()`). **Transition sequence**: this version acquires **both** the new bigint key and the legacy `hashtext()` key (new first, back-off on either failure), so a mixed fleet during a rolling upgrade can never hold two "locks"; the legacy dual-acquire will be dropped in the **next minor** — do not skip this version when upgrading multi-replica deployments. Ops note: `pg_locks` queries keyed on `hashtext('GranitMigration')` should migrate to the new key.

### Changed (framework — behavior)

- **Remaining local-identity endpoints now return localized error details.** Following the `Account*` self-service localization, the rest of `Granit.Identity.Local.Endpoints` (login, two-factor login, registration, email-change, role management, impersonation, session) now resolves translatable keys (`Granit:Identity:Account:*`, `Granit:Identity:Role:*`, `Granit:Identity:TwoFactor:*`, `Granit:Identity:Impersonation:*`) from the `IdentityLocalEndpoints` resource in the request culture (15 base cultures) instead of hardcoded English `detail` strings. Also stops leaking the requested role GUID on role-not-found (`"Role {id} not found."` → localized `"Role not found."`). The `ex.Message` passthroughs in role mutation are unchanged (separate orchestrator-exception concern); the developer-facing 500 "no authentication handler" message stays in English by design.
- **Built-in validation message keys adopt the `Validation:Builtin:*` convention (ADR-066, Phase 1a).** The FluentValidation built-in message keys are renamed in the `Validation` resource (`Validation:NotEmptyValidator` → `Validation:Builtin:NotEmpty`, `Validation:MinimumLengthValidator` → `Validation:Builtin:MinimumLength`, …) and `GranitErrorCodeLanguageManager` maps the bare validator name to the new key. The FluentValidation wire `ErrorCode` is **unchanged** (still `{Rule}Validator`) — only the localization lookup key moves, so resolved 422 messages are identical. The 22 built-in entries leave the `ValidationKeyConventionTests` `PendingMigration` backlog. Clients that resolve these keys client-side (granit-front) must mirror the rename (see granit-front#625).
- **Format/identifier and hint validation keys adopt `Validation:Format:*` / `Validation:Hint:*` (ADR-066, Phase 1b/1c).** The 23 framework format validators are renamed `Validation:Invalid{X}` → `Validation:Format:{X}` (`Validation:InvalidIban` → `Validation:Format:Iban`; `Validation:UrlMustBeHttps` → `Validation:Format:UrlHttps`), the 3 pattern hints `Validation:Hints:*` → `Validation:Hint:*`, and the 422 problem title `Validation:ProblemTitle` → `Validation:Problem:Title`. Unlike the built-ins these are `WithErrorCodeAndMessage` codes, so the wire `ErrorCode` in the 422 `errors` map **does change** for these rules (pre-1.0; no `[Obsolete]` graduation). Emitters (core validator extensions + `CoreServerValidatorContributor`), the resource (18 cultures), and all asserting tests are updated; regional packages (`Granit.Validation.Europe`/`.UnitedKingdom`/`.NorthAmerica`) keep their own codes. Mirror in granit-front#625.
- **Framework validation key migration complete — `PendingMigration` backlog is now empty (ADR-066, Phase 1d).** The 19 business-domain keys with no framework consumer (`CalendarRange*`, `Metering*`, `Payment*`, `PeriodSpec*`, `Regroup*`/`Reorder*`/`TooManyDeltas`, `EntityViewShareEmptyAudience`, `ScopeRequired`/`SearchTermRequired`/`TargetId*`/`TargetType*`, `CodeInvalidFormat`, `MaxMetadataKeys`, `ValidToAfterValidFrom`) are **removed** from the framework `Granit.Validation` resource (18 cultures) — they belong to their owning granit-business modules (relocated in Phase 2; they never resolved framework-side anyway, the consumers use stale `Granit:Validation:*` codes). The one genuinely framework-shared rule, `MaxBatchSize` (used by `Granit.Validation.Endpoints` + `Granit.Identity.Endpoints`), moves to a new `Rule` category: `Validation:MaxBatchSize` → `Validation:Rule:MaxBatchSize`. `ValidationKeyConventionTests` now enforces `Validation:(Builtin|Format|Hint|Problem|Rule):{PascalCase}` with an empty backlog.
