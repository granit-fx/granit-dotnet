using Granit.Domain;

namespace Granit.Customers.Domain.ValueObjects;

/// <summary>Strongly-typed identifier for a <see cref="Customer"/>.</summary>
public sealed class CustomerId : SingleValueObject<Guid>
{
    /// <inheritdoc />
    public override required Guid Value { get; init; }

    /// <summary>Creates a new <see cref="CustomerId"/>.</summary>
    public static CustomerId Create(Guid value)
    {
        if (value == Guid.Empty)
        {
            throw new ArgumentException("Customer identifier must not be empty.", nameof(value));
        }

        return new CustomerId { Value = value };
    }

    /// <summary>Implicit conversion to <see cref="Guid"/>.</summary>
    public static implicit operator Guid(CustomerId id) => id.Value;

    /// <summary>Implicit conversion from <see cref="Guid"/>.</summary>
    public static implicit operator CustomerId(Guid value) => Create(value);
}
