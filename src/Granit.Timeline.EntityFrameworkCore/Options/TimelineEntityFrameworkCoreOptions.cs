...
namespace Granit.Timeline.EntityFrameworkCore.Options;

/// <summary>
/// Configuration shape for
/// <c>TimelineEntityFrameworkCoreHostApplicationBuilderExtensions.AddGranitTimelineEntityFrameworkCore</c>.
/// Carries the dual-scope storage choice and the EF Core
/// <see cref="DbContextOptionsBuilder"/> callbacks per context.
/// </summary>
/// <remarks>
/// <para>
/// Per ADR-063, Granit.Timeline is a dual-scope module: platform-level activity (host
/// announcements, system events, <c>TenantId == null</c>) coexists with per-tenant
/// activity (user comments, reactions, attachments). <see cref="StorageMode"/> selects how
/// the two are physically laid out.
/// </para>
/// <para>
...
/// <c>DROP SCHEMA &lt;tenant&gt; CASCADE</c> lessivage for RGPD Art. 17 — timeline entries
/// carry PII (comment bodies, attachment file names).
/// Requires <see cref="ConfigureHost"/> plus at least one of the tenant-side configurations.
/// </para>
/// </remarks>
public sealed class TimelineEntityFrameworkCoreOptions
{
    ...
    /// <summary>
    /// EF Core configuration for the single shared <c>TimelineHostDbContext</c>.
    ...
    /// <summary>
    /// EF Core configuration for the host-pinned context that holds platform-level entries.
    ...
    /// <summary>
    /// EF Core configuration for <c>TimelineTenantDbContext</c> under
