...
Generic party-management module for Granit. The `Party` aggregate represents any
...
A single contact can hold any combination of `[Flags] PartyRoles` (Customer,
...
Party platformCustomer = Party.Create(
    Guid.NewGuid(), tenantId: null, PartyKind.Company, "ACME Inc.", "EUR");

// Tenant e-commerce app — that tenant's own end-customer base.
Party endCustomer = Party.Create(
    Guid.NewGuid(), tenantId: currentTenant.Id, PartyKind.Individual, "Jean Dupont", "EUR");
```
...
```bash
dotnet add package Granit.Parties
```
