...
/// </summary>
/// <remarks>
/// <see cref="IIdempotencyStore.IsDistributed"/> is part of the contract, so the guard
/// applies uniformly to any implementation — including host-provided custom stores.
/// </remarks>
    ...
    ILogger<IdempotencyStartupGuard> logger) : IHostedService
{
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            LogDistributedStore(logger, store.BackendName);
            return Task.CompletedTask;
        }

        if (environment.IsDevelopment() || options.Value.AllowInMemoryStore)
        {
            LogNonDistributedStore(logger, store.BackendName, environment.EnvironmentName);
            return Task.CompletedTask;
        }

        throw new InvalidOperationException(
            $"Granit idempotency is backed by the non-distributed '{store.BackendName}' in the " +
            $"'{environment.EnvironmentName}' environment. Under multiple replicas the same Idempotency-Key " +
            "can execute twice (double payment, double email). Register a distributed IIdempotencyStore " +
            "(Granit.Http.Idempotency.StackExchangeRedis) or, for a genuinely single-instance deployment, set " +
    ...
    private static partial void LogNonDistributedStore(ILogger logger, string backend, string environment);
}
