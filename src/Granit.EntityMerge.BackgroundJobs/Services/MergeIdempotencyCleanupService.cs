...
/// configured <see cref="EntityMergeOptions.IdempotencyRetention"/> window. Runs as a single
...
internal sealed partial class MergeIdempotencyCleanupService(
    IDbContextFactory<EntityMergeDbContext> contextFactory,
    ...
    ILogger<MergeIdempotencyCleanupService> logger) : IMergeIdempotencySweeper
{
    ...
    [LoggerMessage(
        ...
        Message = "EntityMerge idempotency cache sweep removed {DeletedCount} expired rows (cutoff: {Cutoff:O}).")]
