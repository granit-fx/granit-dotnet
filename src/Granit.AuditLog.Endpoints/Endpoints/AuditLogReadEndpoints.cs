...
namespace Granit.AuditLog.Endpoints.Endpoints;

/// <summary>
/// Read-only Minimal API endpoints for the audit trail.
/// </summary>
internal static class AuditLogReadEndpoints
{
    /// <summary>Maps all audit log read endpoints to the given route group.</summary>
    public static RouteGroupBuilder MapAuditLogReadEndpoints(this RouteGroupBuilder group)
    {
        group.MapGet("/", GetPagedAsync)
            .WithName("GetAuditLogEntries")
            ...
            .Produces<PagedResult<AuditLogEntryResponse>>();

        group.MapGet("/{id:guid}", GetByIdAsync)
            .WithName("GetAuditLogEntryById")
            ...
            .Produces<AuditLogEntryDetailResponse>()
            .ProducesProblem(StatusCodes.Status404NotFound);

        group.MapGet("/entity/{entityType}/{entityId}", GetByEntityAsync)
            .WithName("GetAuditLogEntriesByEntity")
        ...
        CancellationToken cancellationToken)
    {
        ...
        PagedResult<AuditLogEntryResponse> mapped = new(
            result.Items.Select(AuditLogResponseMapper.ToSummaryResponse).ToList(),
            result.TotalCount,
            result.HasMore);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return TypedResults.Ok(AuditLogResponseMapper.ToDetailResponse(entry));
    }

    private static async Task<Ok<PagedResult<AuditLogEntryResponse>>> GetByEntityAsync(
        string entityType,
        string entityId,
        [FromQuery] int? page,
