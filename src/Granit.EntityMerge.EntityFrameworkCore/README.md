...
EF Core orchestrator for the Granit merge primitive (`Granit.EntityMerge`). Concrete
...
| Type | Purpose |
|---|---|
| `EfMergeService<TAggregate>` | Open-generic concrete `IMergeService<TAggregate>`. Registered by `AddGranitEntityMergeEntityFrameworkCore` and resolved per aggregate type by consuming modules. |
| `EntityMergeDbContext` | Isolated DbContext owning the `granit.merge_idempotency` cache table. |
| `MergeIdempotencyEntry` | Internal cache row. |
| `EntityMergeConcurrencyLock` | Postgres `pg_advisory_xact_lock` / SQL Server `sp_getapplock` helper, mirroring the `Granit.Metering` pattern. |
| `MergeRequestHasher` | Canonicalises a `MergeRequest` into a stable SHA-256 digest used as the replay key. |
| `EntityMergeEntityFrameworkCoreHostApplicationBuilderExtensions.AddGranitEntityMergeEntityFrameworkCore` | Wires the DbContext + the open-generic `EfMergeService`. |
...
```csharp
builder.AddGranitEntityMergeEntityFrameworkCore(opt => opt.UseNpgsql(connectionString));

// Per consuming module (e.g. Granit.Parties.EntityMerge):
