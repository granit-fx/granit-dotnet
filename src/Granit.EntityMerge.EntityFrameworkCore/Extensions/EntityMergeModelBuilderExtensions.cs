...
/// <summary>
/// ModelBuilder extension exposing the EntityMerge-orchestrator entity configurations so host
...
/// </summary>
/// <remarks>
/// Table and schema are configurable via <see cref="GranitEntityMergeDbProperties"/>; the
/// schema defaults to <see cref="GranitDbDefaults.HostDbSchema"/> (the orchestrator state is
/// host-level, never tenant-scoped).
/// </remarks>
public static class EntityMergeModelBuilderExtensions
{
    /// <summary>
    /// Registers the merge-orchestrator entities + indexes on the provided <paramref name="modelBuilder"/>.
    /// </summary>
    public static ModelBuilder ConfigureEntityMergeModule(this ModelBuilder modelBuilder)
    {
        ...
        {
            b.ToTable(
                GranitEntityMergeDbProperties.DbTablePrefix + "idempotency",
                GranitEntityMergeDbProperties.DbSchema);
            b.HasKey(e => e.Id);
