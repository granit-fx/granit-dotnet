...
using Granit.Http.ApiDocumentation.Scalar.Internal;
...
using DocOAuth2Options = Granit.Http.ApiDocumentation.Options.OAuth2Options;
using GranitScalarOptions = Granit.Http.ApiDocumentation.Scalar.Options.ScalarOptions;
...
{
    /// <summary>
    /// Maps OpenAPI JSON endpoints (<c>/openapi/v{n}.json</c>, via
    /// <c>MapGranitOpenApiDocuments</c>) and the Scalar interactive UI.
    /// Always enabled in Development; in Production only when
    /// <c>Http:ApiDocumentation:Scalar:EnableInProduction</c> is <c>true</c>.
    /// When <c>Http:ApiDocumentation:Scalar:AuthorizationPolicy</c> is set, the endpoints
    ...
    public static WebApplication UseGranitApiDocumentation(this WebApplication app)
    {
        ...
        GranitScalarOptions uiOptions =
            app.Services.GetRequiredService<IOptions<GranitScalarOptions>>().Value;
        ApiDocumentationOptions docOptions =
            app.Services.GetRequiredService<IOptions<ApiDocumentationOptions>>().Value;

        bool shouldEnable = app.Environment.IsDevelopment() || uiOptions.EnableInProduction;
        if (!shouldEnable)
        {
            return app;
        }
        ...
        if (!app.Environment.IsDevelopment()
            && uiOptions.EnableInProduction
            && uiOptions.AuthorizationPolicy is null)
        {
            LogProductionOpenApiWithoutPolicy(CreateLogger(app));
        }

        app.MapGranitOpenApiDocuments(
            endpoint => ApplyAuthorizationPolicy(endpoint, uiOptions.AuthorizationPolicy));

        bool oauth2PopupEnabled = IsOAuth2PopupEnabled(docOptions, uiOptions);

        IEndpointConventionBuilder scalarEndpoint = app.MapScalarApiReference(
            scalarOptions => ConfigureScalar(scalarOptions, docOptions, uiOptions, oauth2PopupEnabled));
        ...
        if (oauth2PopupEnabled)
        {
            scalarEndpoint.WithMetadata(AllowsPopupAuthorizationMetadata.Instance);
        }

        ApplyAuthorizationPolicy(scalarEndpoint, uiOptions.AuthorizationPolicy);
    ...
    /// <summary>
    /// The interactive OAuth2 flow needs both halves: the endpoint URLs from the
    /// generation-side options and the public client id from the UI-side options.
    /// </summary>
    private static bool IsOAuth2PopupEnabled(
        ApiDocumentationOptions docOptions,
        GranitScalarOptions uiOptions) =>
        docOptions.OAuth2.IsConfigured
        && !string.IsNullOrEmpty(uiOptions.OAuth2.ClientId);

    private static void ConfigureScalar(
        ScalarOptions scalarOptions,
        ApiDocumentationOptions docOptions,
        GranitScalarOptions uiOptions,
        bool oauth2PopupEnabled)
    {
        scalarOptions.WithTitle(docOptions.Title);

        if (!string.IsNullOrEmpty(uiOptions.FaviconUrl))
        {
            scalarOptions.WithFavicon(uiOptions.FaviconUrl);
        }

        if (oauth2PopupEnabled)
        {
            scalarOptions.AddAuthorizationCodeFlow(
                "OAuth2",
                flow => ConfigureOAuth2Flow(flow, docOptions.OAuth2, uiOptions.OAuth2));
        }
    }

    private static void ConfigureOAuth2Flow(
        AuthorizationCodeFlow flow,
        DocOAuth2Options docOAuth2,
        Options.ScalarOAuth2Options uiOAuth2)
    {
        flow
            .WithClientId(uiOAuth2.ClientId)
            .WithSelectedScopes(docOAuth2.Scopes);

        if (uiOAuth2.EnablePkce)
        {
            flow.WithPkce(Pkce.Sha256);
        }

        if (!string.IsNullOrEmpty(uiOAuth2.RedirectUri))
        {
            flow.WithRedirectUri(uiOAuth2.RedirectUri);
        }
    }
    ...
    /// consumer references <c>Granit.Http.ApiDocumentation.Scalar</c> without
    ...
    private static void RegisterScalarCspContributor(WebApplication app)
    {
        ...
        if (registry is null)
        {
            LogCspRegistryUnresolved(CreateLogger(app));
            return;
        }
    ...
    private static ILogger CreateLogger(WebApplication app) =>
        app.Services
            ...
            .CreateLogger("Granit.Http.ApiDocumentation.Scalar");
    ...
    [LoggerMessage(
        ...
        Message = "OpenAPI/Scalar is exposed in production with no AuthorizationPolicy configured. " +
                  ...
                  "Set Http:ApiDocumentation:Scalar:AuthorizationPolicy to a named policy " +
                  "(e.g. \"ApiDocsReaders\") or an empty string (explicit anonymous) to silence this warning.")]
