    ...
    IMobilePushEventPublisher eventPublisher) : IMobilePushSender
{
    ...
    public async Task SendAsync(MobilePushMessage message, CancellationToken cancellationToken = default)
    {
        ...
        using Activity? activity = NotificationsAwsSnsMobilePushActivitySource.Source.StartActivity(
            NotificationsAwsSnsMobilePushActivitySource.Operations.Send);
        activity?.SetTag(NotificationsAwsSnsMobilePushActivitySource.Tags.DeviceCount, message.DeviceTokens.Count);
