...
/// provider from spawning a weaponised browser binary out of a writable location.
/// </summary>
        ...
        if (string.IsNullOrEmpty(allowedPrefix))
        {
            if (hostEnvironment is not null && hostEnvironment.IsProduction())
            {
                throw new SandboxViolationException(
                    ...
                    $"IBrowserSandboxProfile.AllowedExecutablePathPrefix must be set when overriding ExecutablePath ('{resolved}') in production.");
            }
            return resolved;
        }
