...
/// </summary>
/// <remarks>
/// The Nominatim provider passes the address in the request query string (<c>?street=…&amp;city=…</c>), which the
/// instrumentation records verbatim in the span's <c>url.full</c> / <c>url.query</c> tags — leaking personal data
/// (GDPR) into trace exporters. This handler sits inside the diagnostics handler in the <c>HttpClient</c> pipeline,
