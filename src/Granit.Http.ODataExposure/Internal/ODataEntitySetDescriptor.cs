...
/// <param name="ExpandWhitelist">Allowed dotted navigation paths for <c>$expand</c> (<c>"Customer"</c>, <c>"Customer.Address"</c>) — prefixes of a whitelisted path are implicitly allowed. <see langword="null"/> means <c>$expand</c> is disabled entirely. Empty list (<c>[]</c>) ALSO disables expand — the EntitySet's contract is "no navigation by default; opt in explicitly per path". Validated at startup: every segment must exist as a navigation on its CLR type and every reachable target type needs a registered <c>ExportDefinition</c> (ADR-050 / #3005).</param>
/// <param name="MaxExpansionDepth">Maximum nesting depth allowed for <c>$expand</c>. Default <c>1</c> — flat expand only; nested expand requires explicit opt-in. Enforced per request by the AST validator AND by <c>ODataValidationSettings.MaxExpansionDepth</c> (#3005).</param>
...
/// <param name="ExpandConfigurationAcknowledged">Set by <see cref="Options.ODataEntitySetBuilder{TEntity}.ExpandWhitelist"/>, <see cref="Options.ODataEntitySetBuilder{TEntity}.DisableExpand"/>, or implicit via <c>Options.ODataEntitySetBuilder{TEntity}.AllowNoExpansion</c>. The strict-config validator refuses to start the host until one of these is called — implicit "expand disabled" is a security smell, the host must declare the intent.</param>
