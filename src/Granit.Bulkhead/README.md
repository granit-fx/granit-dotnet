...
single tenant from monopolizing server resources (CPU, memory, threads). Framework-pure core (ADR-062):
the ASP.NET Core endpoint filter ships in `Granit.Http.Bulkhead`, the Wolverine
middleware in `Granit.Bulkhead.Wolverine`.
