    ...
    ILogger<StripeTaxCalculator> logger) : ITaxCalculator
{
        ...
        TaxRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            Log.CalculationError(logger, ex, ex.StripeError?.Code);
            return new TaxResult([], TotalTax: 0m, Jurisdiction: null);
        }
    }
    ...
    private static partial class Log
    {
        ...
        [LoggerMessage(Level = LogLevel.Warning, Message = "Stripe Tax calculation error: {ErrorCode}")]
        public static partial void CalculationError(ILogger logger, Exception ex, string? errorCode);
    }
}
