...
internal sealed class DefaultUrlSafetyValidator : IUrlSafetyValidator
{
    ...
    private readonly UrlSafetyMetrics _metrics;
    ...
    public async ValueTask<UrlSafetyResult> ValidateAsync(Uri url, UrlSafetyOptions optionOverrides, CancellationToken ct = default)
    {
        ...
        using Activity? activity = UrlSafetyActivitySource.Source.StartActivity(UrlSafetyActivitySource.ValidateUrl);
    ...
    {
        UrlSafetyLog.DnsResolutionFailed(_logger, SanitizeForDisplay(asciiHost), failureTag);
        return new UrlSafetyViolation(
    ...
    private void RecordValid() => _metrics.RecordValid();

    private UrlSafetyResult Block(Activity? activity, UrlSafetyViolation violation)
    {
        _metrics.RecordBlocked(violation.Kind);
        UrlSafetyLog.UrlBlocked(_logger, RedactHostForLog(ExtractHost(violation)), violation.Kind, violation.Reason);
