...
> ⚠️ **NEVER `dotnet build`/`test`/`format` on the full solution.** Roslyn OOMs. Target a `.slnf` shard or single project.
...
- Handler injecting an `internal` service → make it public or extract an interface.
...
- Framework baseline ships **EN + FR**. Other cultures via `scripts/translate-templates.py` with `<!-- AUTO-TRANSLATED -->` marker.
...
- Errors: `TypedResults.Problem(detail, statusCode)` (RFC 7807). NEVER return EF entities — project to `*Response` records.
...
Every `*.Endpoints` module attaches `.WithTags(...)` on its root group. Format: `Title Case With Spaces` (`Blob Storage`, `Background Jobs`) — NEVER glued PascalCase, kebab-case, or snake_case. Multi-tag: `<Module> - <SubGroup>` (space-dash-space) — `AI - Workspaces`. Expose via `TagName` on `*EndpointsOptions` (overridable per-app). `Granit.Http.ApiDocumentation` auto-emits a sorted `document.Tags` array.
...
2. **DbContext owning ≥1 `IMultiTenant` entity → inherit `GranitDbContext`** (forward `options, currentTenant, dataFilter` to base). Base exposes `CurrentTenantId` / `IsMultiTenantFilterEnabled` as instance members so EF Core parameterises the tenant filter (`@ef_filter__CurrentTenantId`) instead of inlining a frozen constant — a closure-captured tenant leaks across requests (repro: `MultiTenantFilterParameterizationReproTests.cs`). With **no `IMultiTenant` entity**, inheriting `DbContext` directly is the default — but inheriting `GranitDbContext` is also allowed to get `ApplyGranitConventions` free (enum-as-string, soft-delete, active, …): the leak hazard cannot arise without a tenant entity (base passes `currentTenant: null`, same as a direct call). Document the choice inline.
3. Constructor-inject `ICurrentTenant?` and `IDataFilter?` (both optional, default `null`).
4. Override `OnGranitModelCreating` (not `OnModelCreating`) on `GranitDbContext` derivatives. Do **NOT** call `ApplyGranitConventions` there — since the native-conventions flip (#3159) the base class owns every pass: this-bound named filters, finalizing conventions (enum-as-string, concurrency stamp, tombstone, ownership indexes, translations), and the value-object trio. Extra model-configuration conventions go through `OnGranitConfigureConventions`. NO manual `HasQueryFilter`. (`ApplyGranitConventions` remains only for plain-`DbContext` contexts, with the #3174 flow-scoped-disable limitation, until Phase 3.)
...
Every enum property on an entity is persisted as its **PascalCase string name in a `varchar` column** by `ApplyGranitConventions` — no `.HasConversion<string>()` in `*Configuration.cs`. Default column width `max(20, longestValueName + 4)`; explicit `.HasMaxLength(N)` wins. Rationale: ops/SQL-audit readability, refactor safety, symmetry with wire format (`JsonStringEnumConverter`).
...
Aggregate Root rules (`DomainConventionTests`):
