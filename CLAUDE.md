...
4. Override `OnGranitModelCreating` (not `OnModelCreating`) on `GranitDbContext` derivatives — the base class owns every pass: this-bound named filters, finalizing conventions (enum-as-string, concurrency stamp, tombstone, ownership indexes, translations), and the value-object trio. Extra model-configuration conventions go through `OnGranitConfigureConventions`. NO manual `HasQueryFilter`. `ApplyGranitConventions` is **internal** since #3162 — every concrete `*DbContext` MUST derive `GranitDbContext` (archi-tested; sole exemption: `MigrationProgressDbContext`, tenant-agnostic system infra).
