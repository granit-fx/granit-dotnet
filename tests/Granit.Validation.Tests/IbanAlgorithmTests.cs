...
public sealed class IbanAlgorithmTests
{
    ...
    public void IsValid_ValidIban_ReturnsTrue(string iban)
    {
        IbanAlgorithm.IsValid(iban).ShouldBeTrue();
    }
    ...
    public void IsValid_InvalidIban_ReturnsFalse(string? iban)
    {
        IbanAlgorithm.IsValid(iban).ShouldBeFalse();
    }
}
