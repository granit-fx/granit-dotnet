...
/// <summary>
/// Creates <see cref="WebPushDbContext"/> instances backed by SQLite in-memory
...
/// </remarks>
internal sealed class TestDbContextFactory : IDbContextFactory<WebPushDbContext>, IDisposable
{
    private readonly SqliteConnection _connection;
    private readonly DbContextOptions<WebPushDbContext> _options;
    ...
    public static TestDbContextFactory Create()
    {
        ...
        DbContextOptionsBuilder<WebPushDbContext> optionsBuilder = new();
        optionsBuilder.UseSqlite(connection);
        optionsBuilder.ReplaceService<IModelCustomizer, SqliteCompatibleModelCustomizer>();

        DbContextOptions<WebPushDbContext> options = optionsBuilder.Options;

        DataFilter dataFilter = new();
        IDisposable multiTenantDisabled = dataFilter.Disable<IMultiTenant>();

        // Create the schema
        using (WebPushDbContext db = new(options, new PassthroughEncryption(), GranitDesignTime.CurrentTenant, dataFilter))
        {
    ...
    public WebPushDbContext CreateDbContext() => new(_options, new PassthroughEncryption(), GranitDesignTime.CurrentTenant, _dataFilter);
