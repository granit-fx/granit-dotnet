using Granit.BackgroundJobs;
using Granit.OpenIddict.BackgroundJobs.Jobs;
using Shouldly;
using Xunit;

namespace Granit.OpenIddict.BackgroundJobs.Tests.Jobs;

public sealed class OpenIddictTokenCleanupJobTests
{
    [Fact]
    public void Should_have_RecurringJob_attribute_with_hourly_cron()
    {
        var attr = (RecurringJobAttribute?)Attribute.GetCustomAttribute(
            typeof(OpenIddictTokenCleanupJob), typeof(RecurringJobAttribute));

        attr.ShouldNotBeNull();
        attr!.CronExpression.ShouldBe("0 * * * *");
        attr.Name.ShouldBe("openiddict-token-cleanup");
    }

    [Fact]
    public void Should_implement_IBackgroundJob() =>
        typeof(IBackgroundJob).IsAssignableFrom(typeof(OpenIddictTokenCleanupJob)).ShouldBeTrue();
}
