...
namespace Granit.AI.Chat.Identity.Tests;
...
public sealed class UserMentionResolverTests
{
    ...
    public async Task Resolve_returns_null_when_the_user_is_absent()
    {
        ...
        AIMentionContext? context = await Build().ResolveAsync("ghost", TestContext.Current.CancellationToken);

        context.ShouldBeNull();
    }

    [Fact]
    public async Task Resolve_maps_a_found_user_to_context()
    {
        ...
        AIMentionContext? context = await Build().ResolveAsync("u1", TestContext.Current.CancellationToken);

        context.ShouldNotBeNull();
        context.Type.ShouldBe("user");
        context.Id.ShouldBe("u1");
        context.Label.ShouldBe("Ada Lovelace");
        context.Content.ShouldContain("ada@x.io");
        context.Content.ShouldContain("Username: ada");
    }
