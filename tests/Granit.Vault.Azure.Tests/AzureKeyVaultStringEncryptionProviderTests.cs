...
public sealed class AzureKeyVaultStringEncryptionProviderTests
{
    private readonly ITransitEncryptionService _transitEncryption =
        Substitute.For<ITransitEncryptionService>();

    private readonly AzureKeyVaultStringEncryptionProvider _sut;

    public AzureKeyVaultStringEncryptionProviderTests()
    {
        StringEncryptionOptions encOptions = new() { VaultKeyName = "test-key" };

        _sut = new AzureKeyVaultStringEncryptionProvider(
            _transitEncryption,
            Microsoft.Extensions.Options.Options.Create(encOptions));
    }

    [Fact]
    public void ProviderName_ReturnsAzureKeyVault() =>
        _sut.ProviderName.ShouldBe("AzureKeyVault");
