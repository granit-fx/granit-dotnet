...
public sealed class EventBusMetricsTests : IDisposable
{
    private readonly ServiceProvider _sp;
    private readonly IMeterFactory _meterFactory;
    private readonly EventBusMetrics _metrics;

    public EventBusMetricsTests()
    {
        ServiceCollection services = new();
        services.AddMetrics();
        _sp = services.BuildServiceProvider();
        _meterFactory = _sp.GetRequiredService<IMeterFactory>();
        _metrics = new EventBusMetrics(_meterFactory);
    }
    ...
    public void RecordEventPublished_IncrementsWithCorrectTags()
    {
        using var collector = new MetricCollector<long>(
            _meterFactory, EventBusMetrics.MeterName, "granit.eventbus.events.published");
    ...
    {
        using var collector = new MetricCollector<long>(
            _meterFactory, EventBusMetrics.MeterName, "granit.eventbus.handlers.executed");
