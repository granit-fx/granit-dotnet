...
/// <summary>
/// Locks the contract of <see cref="EfUserDirectoryWriter"/> — inserts and
/// hard-deletes <see cref="User"/> rows through a short-lived
/// <see cref="IdentityDbContext"/>. SQLite in-memory keeps the test fast
...
public sealed class EfUserDirectoryWriterTests : IDisposable
{
    ...
    {
        EfUserDirectoryWriter writer = new(_factory);
        var id = Guid.NewGuid();
    ...
    {
        // The User aggregate is NOT soft-deletable (per ADR-051 the
        // canonical user is the GDPR drop point) — DeleteAsync must
        // erase the row entirely, not flip an IsDeleted flag.
        EfUserDirectoryWriter writer = new(_factory);
        var id = Guid.NewGuid();
    ...
    {
        // Compensating delete must tolerate the case where the User row
        // was never created (e.g. the upstream LocalIdentity insert failed
        // before the writer was even called).
        EfUserDirectoryWriter writer = new(_factory);
