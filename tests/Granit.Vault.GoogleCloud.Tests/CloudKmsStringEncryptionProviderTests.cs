...
public sealed class CloudKmsStringEncryptionProviderTests
{
    private readonly ITransitEncryptionService _transitEncryption = Substitute.For<ITransitEncryptionService>();
    private readonly CloudKmsStringEncryptionProvider _sut;

    public CloudKmsStringEncryptionProviderTests()
    {
        StringEncryptionOptions encOptions = new() { VaultKeyName = "test-key" };

        _sut = new CloudKmsStringEncryptionProvider(
            _transitEncryption,
            Microsoft.Extensions.Options.Options.Create(encOptions));
    }

    [Fact]
    public void ProviderName_ReturnsGoogleCloudKms() =>
        _sut.ProviderName.ShouldBe("GoogleCloudKms");
