...
namespace Granit.EntityMerge.EntityFrameworkCore.Tests;
...
public sealed class EfMergeServiceTests
{
    ...
    private readonly IDbContextFactory<EntityMergeDbContext> _factory;
    private readonly IGuidGenerator _guidGenerator = Substitute.For<IGuidGenerator>();
    private readonly IClock _clock = Substitute.For<IClock>();
    private readonly FakeStringEncryption _encryption = new();
    private readonly IEntityMergeSecretProvider _secretProvider = new FakeSecretProvider();
    private readonly IOptions<EntityMergeOptions> _options = Microsoft.Extensions.Options.Options.Create(new EntityMergeOptions());
    ...
    public async Task MergeAsync_TamperedCacheRow_FallsThroughToFreshMerge()
    {
        ...
        await using (EntityMergeDbContext db = await _factory.CreateDbContextAsync(TestContext.Current.CancellationToken))
        {
    ...
    private sealed class FakeSecretProvider : IEntityMergeSecretProvider
    {
    ...
    private sealed class InMemoryFactory : IDbContextFactory<EntityMergeDbContext>
    {
        private readonly DbContextOptions<EntityMergeDbContext> _options =
            new DbContextOptionsBuilder<EntityMergeDbContext>()
                .UseInMemoryDatabase($"mergeable-tests-{Guid.NewGuid()}")
                .ConfigureWarnings(w =>
                    w.Ignore(Microsoft.EntityFrameworkCore.Diagnostics.InMemoryEventId.TransactionIgnoredWarning))
                .Options;

        public EntityMergeDbContext CreateDbContext() => new(_options, GranitDesignTime.CurrentTenant);
        public Task<EntityMergeDbContext> CreateDbContextAsync(CancellationToken cancellationToken = default) =>
            Task.FromResult(new EntityMergeDbContext(_options, GranitDesignTime.CurrentTenant));
    }
}
