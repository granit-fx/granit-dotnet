...
public sealed class AddressTelemetryRedactionHandlerTests
{
    ...
    public async Task SendAsync_RedactsAddress_FromHttpClientActivityUrlTags()
    {
        ...
        activity.SetTag("url.full", $"https://nominatim.openstreetmap.org/{AddressQuery}");
        activity.SetTag("url.query", "street=Rue%20de%20la%20Loi&city=Brussels&country=BE");

        await SendThroughHandlerAsync($"https://nominatim.openstreetmap.org/{AddressQuery}");

        activity.GetTagItem("url.full").ShouldBe("https://nominatim.openstreetmap.org/search?redacted");
    ...
    public async Task SendAsync_RedactsAddress_FromLegacyHttpUrlTag()
    {
        ...
        using ActivitySource source = new("System.Net.Http");
        using Activity activity = source.StartActivity("System.Net.Http.HttpRequestOut")!;
        activity.SetTag("http.url", $"https://nominatim.openstreetmap.org/{AddressQuery}");

        await SendThroughHandlerAsync($"https://nominatim.openstreetmap.org/{AddressQuery}");

        activity.GetTagItem("http.url").ShouldBe("https://nominatim.openstreetmap.org/search?redacted");
    }
