...
/// <summary>
/// Creates <see cref="MobilePushDbContext"/> instances backed by SQLite in-memory
/// for fast, isolated integration tests that support all relational operations
/// (including <c>ExecuteDeleteAsync</c>).
/// </summary>
/// <remarks>
...
/// </remarks>
internal sealed class TestDbContextFactory : IDbContextFactory<MobilePushDbContext>, IDisposable
{
    private readonly SqliteConnection _connection;
    private readonly DbContextOptions<MobilePushDbContext> _options;
    ...
    public static TestDbContextFactory Create()
    {
        ...
        DbContextOptionsBuilder<MobilePushDbContext> optionsBuilder = new();
        optionsBuilder.UseSqlite(connection);
        optionsBuilder.ReplaceService<IModelCustomizer, SqliteCompatibleModelCustomizer>();

        DbContextOptions<MobilePushDbContext> options = optionsBuilder.Options;

        DataFilter dataFilter = new();
        IDisposable multiTenantDisabled = dataFilter.Disable<IMultiTenant>();

        // Create the schema
        using (MobilePushDbContext db = new(options, new PassthroughEncryption(), GranitDesignTime.CurrentTenant, dataFilter))
        {
    ...
    public MobilePushDbContext CreateDbContext() => new(_options, new PassthroughEncryption(), GranitDesignTime.CurrentTenant, _dataFilter);
