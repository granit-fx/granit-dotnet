...
public sealed class AzureBlobKeyStrategyTests
{
    ...
    private readonly AzureBlobKeyStrategy _sut;

    public AzureBlobKeyStrategyTests()
    {
        _currentTenant.IsAvailable.Returns(true);
        _currentTenant.Id.Returns(TenantId);
        _clock.Now.Returns(Now);

        _sut = new AzureBlobKeyStrategy(
            _currentTenant,
            _clock,
            Microsoft.Extensions.Options.Options.Create(new AzureBlobOptions { DefaultContainer = "granit-blobs" }));
    }
    ...
    public void BuildObjectKey_TwoTenantsWithSameFileName_ShouldProduceDifferentKeys()
    {
        ...
        AzureBlobKeyStrategy strategyB = new(
    ...
    [Fact]
    public void ResolveBucketName_ShouldReturnConfiguredDefaultContainer()
    {
        string container = _sut.ResolveBucketName("medical-images");

        container.ShouldBe("granit-blobs");
    }

    [Fact]
    public void ResolveBucketName_ShouldReturnSameContainerRegardlessOfLogicalName()
    {
