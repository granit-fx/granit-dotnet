...
public sealed class ScalarCspContributorTests
{
    ...
    [Fact]
    public async Task UseGranitApiDocumentation_DoesNotAttachPopupMarker_WhenClientIdMissing()
    {
        // Generation-side URLs configured but no UI-side ClientId → Scalar cannot
        // start the Authorization Code flow, so no popup marker and strict COOP.
        WebApplication app = BuildApp(
            Environments.Development,
            withSecurityHeaders: true,
            configureOAuth2: true,
            configureUiClientId: false);
        app.UseGranitApiDocumentation();
        await app.StartAsync(TestContext.Current.CancellationToken);
        try
        {
            ...
            scalar.ShouldNotBeNull();
            scalar.Metadata.GetMetadata<AllowsPopupAuthorizationMetadata>()
                .ShouldBeNull("Without a UI client id there is no popup flow — COOP must stay at the strict default.");
        }
        finally
        {
            await app.StopAsync(TestContext.Current.CancellationToken);
        ...
        bool configureUiClientId = true)
    {
        ...
        Dictionary<string, string?> config = new()
        {
            ...
            ["Http:ApiDocumentation:Scalar:EnableInProduction"] = enableScalarInProduction.ToString(),
        };
        if (configureOAuth2)
        {
            ...
            if (configureUiClientId)
            {
                config["Http:ApiDocumentation:Scalar:OAuth2:ClientId"] = "scalar-test";
            }
        }
