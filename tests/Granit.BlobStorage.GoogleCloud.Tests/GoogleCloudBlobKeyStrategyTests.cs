...
public sealed class GoogleCloudBlobKeyStrategyTests
{
    ...
    private readonly GoogleCloudBlobKeyStrategy _sut;

    public GoogleCloudBlobKeyStrategyTests()
    {
        _currentTenant.IsAvailable.Returns(true);
        _currentTenant.Id.Returns(TenantId);
        _clock.Now.Returns(Now);

        _sut = new GoogleCloudBlobKeyStrategy(
            _currentTenant,
            _clock,
            Microsoft.Extensions.Options.Options.Create(new GoogleCloudStorageOptions { DefaultBucket = "granit-blobs" }));
    }
    ...
    public void BuildObjectKey_TwoTenantsWithSameFileName_ShouldProduceDifferentKeys()
    {
        ...
        GoogleCloudBlobKeyStrategy strategyB = new(
