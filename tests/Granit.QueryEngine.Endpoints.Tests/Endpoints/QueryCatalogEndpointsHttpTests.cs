...
namespace Granit.QueryEngine.Endpoints.Tests.Endpoints;
...
public sealed class QueryCatalogEndpointsHttpTests
{
    ...
    public async Task Catalog_carries_the_module_derived_from_the_entity_assembly()
    {
        ...
        // Both test entities live in this test assembly (Granit.QueryEngine.Endpoints.Tests), so
        // the default derivation strips the "Granit." prefix to "QueryEngine.Endpoints.Tests".
        QueryCatalogEntryResponse routed = body!.Single(e => e.Name == "Test.Routed");
        routed.ModuleName.ShouldBe("QueryEngine.Endpoints.Tests");
    }
